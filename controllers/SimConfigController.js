...
const logAction = async (userId, workspaceId, entityId, action, details = {}) => { //details provided default empty object
    try {
        await AuditLog.create({ // creates new audit log entry
            user_id: userId, //mappings
            workspace_id: workspaceId,
            entity_type: 'config', //this log is for a config identity
            entity_id: entityId,
            action,
            details,
            timestamp: new Date()
...
exports.createConfig = async (req, res) => { //exports as a route handler
    try {
        ...
        const config = await SimulationConfig.create({ //creats new config in database
            ...
            is_adaptive,
            adaptive_settings: is_adaptive ? adaptive_setting : undefined, //only save if enabled
            created_at: new Date(),
...
exports.getMyConfigs = async(req, res) => {
    try {
        ...
        const configs = await SimulationConfig.find({workspace_id: { $in: workspaceIds } //all configs where workspace_id is in the user's workspace IDs
        })
...
exports.getConfigsByWS = async(req, res) => {
    ...
    exports.getTemplates = async (req, res) => {
        try{
            ...
            const templates = await SimulationConfig.find({workspace_id: { $in: workspaceIds}, // finds configs in user's workspaces
                is_template: true
...
exports.deleteConfig = async (req, res) => {
    try {
        ...
        await SimulationConfig.deleteOne({config_id: configId}); //deletes the config from DB

        await logAction(userId, workspaceId, configId, 'delete', {name: config.name}); //logs deletion
